...
On our servers, we use either [montagu-config](https://github.com/vimc/montagu-config) or [packit-config](https://github.com/mrc-ide/packit-config) to provide the configuration file `packit.yml`. (When both montagu and packit are installed on the same server, `montagu-config` contains configuration files for both montagu and packit. If the server is packit-only, then we use `packit-config` instead, containing configuration for packit only.)

To use packit-deploy on our servers:
...
1. If there has been a change to packit-deploy or the config repo, use [infra-scripts](https://github.com/reside-ic/infra-scripts/blob/main/PACKIT.md) to install the latest version of the deploy tool and clone the config repo.
1. SSH into the server you want to update.
